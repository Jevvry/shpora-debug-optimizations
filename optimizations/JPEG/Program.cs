...
namespace JPEG
{
    class Program
    {
        ...
        {
            quality = 3;
            var transformAlgorithm = new SuperFastFFT();

            var lineSize = (matrix.Width / DCTSize) * DCTSize * DCTSize * 3;
            var bytesLenght = (matrix.Height / DCTSize) * lineSize;
            var bytes = new byte[bytesLenght];
            Parallel.For(0, matrix.Height / DCTSize, k =>
            {
                var y = DCTSize * k;
                var count = 0;
        ...
        {
            var result = new Matrix(image.Height, image.Width, new Bitmap(image.Width, image.Height,
                System.Drawing.Imaging.PixelFormat.Format24bppRgb));
            var transformAlgorithm = new SuperFastFFT();
            var bytes = HuffmanCodec.Decode(image.CompressedBytes, image.DecodeTable, image.BitsCount);
            var lineSize = (image.Width / DCTSize) * DCTSize * DCTSize * 3;
            Parallel.For(0, image.Height / DCTSize, k =>
               {
                   var y = DCTSize * k;
                   var count = 0;
                   for (var x = 0; x < image.Width; x += DCTSize)
                   {
                       ...
                       {
                           var quantizedBytes = new ArraySegment<byte>(
                               bytes, lineSize * k + DCTSize * DCTSize * count, DCTSize * DCTSize);
                           var quantizedFreqs = ZigZagUnScan(quantizedBytes);
                           var channelFreqs = DeQuantize(quantizedFreqs, image.Quality);
                           FFT2D.IFft2D(channelFreqs, channel, transformAlgorithm);
        ...
        private static byte[,] ZigZagUnScan(ArraySegment<byte> quantizedBytes)
        {
            return new[,]
            {
        ...
        private static int[,] GetQuantizationMatrix(int quality)
        {
            ...
            if (quantizationMatrix.Length != 0)
            ...
            for (int y = 0; y < quantizationMatrix.GetLength(0); y++)
            {
                for (int x = 0; x < quantizationMatrix.GetLength(1); x++)
                {
        ...
        private static int[,] quantizationMatrix = new int[0, 0];
        const int DCTSize = 8;
    }
}
